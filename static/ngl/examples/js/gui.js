...
NGL.MenubarWidget = function( stage, preferences ){
    ...
    container.add( new NGL.MenubarVisualizationWidget( stage ) );
...
NGL.MenubarVisualizationWidget = function( stage ){
    ...
    return UI.MenubarHelper.createMenuContainer( 'Visualize', optionsPanel );

};


NGL.MenubarDockingWidget = function( stage ){
    ...
    var proteinUpload = document.createElement("input");
    proteinUpload.type = "file";
    proteinUpload.multiple = true;
    proteinUpload.style.display = "none";
    proteinUpload.accept = "." + fileTypesOpen.join( ",." );
    proteinUpload.addEventListener( 'change', proteinUploadOnChange, false );

    var ligandUpload = document.createElement("input");
    ligandUpload.type = "file";
    ligandUpload.multiple = true;
    ligandUpload.style.display = "none";
    ...
    function uploadFiles(){
        ...
        console.log(data.get('proteins'))
        console.log(data.get('ligands'))
        jQuery.ajax({
            url: '/upload',
            data: data,
            cache: false,
            ...
            success: function(data) {
                ...
                for(var i=0; i < proteins.length; i++){
                    for(var j=0; j < ligands.length; j++){
                        stage.loadFile( docking_result[i][j]["protein"], {
                            defaultRepresentation: true
                        } )
                        stage.loadFile( docking_result[i][j]["ligand"], {
                            defaultRepresentation: true
                        } )
                    }
                }
    ...
    var optionsPanel = UI.MenubarHelper.createOptionsPanel( menuConfig );
    optionsPanel.dom.appendChild( proteinUpload );
    optionsPanel.dom.appendChild( ligandUpload );

    return UI.MenubarHelper.createMenuContainer( 'Dock', optionsPanel );

};


NGL.MenubarSettingsWidget = function( stage, preferences ){
    ...
    return UI.MenubarHelper.createMenuContainer( 'Settings', optionsPanel );
